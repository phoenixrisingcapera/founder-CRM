...
import type { DeckExport, DeckGraph } from '$types/domain';
...
export async function load({ params, fetch }) {
  ...
  const exportsResponse = await fetch(`/api/decks/${params.deckId}/exports`);
  const exportsPayload = exportsResponse.ok ? ((await exportsResponse.json()) as { exports?: DeckExport[] }) : { exports: [] };
