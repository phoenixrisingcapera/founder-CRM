...
<div class="page-grid"><SectionCard title="Venture Goals" subtitle="The active founder outcomes this CRM is helping you win."><div class="form-grid"><input class="field" bind:value={form.title} placeholder="Venture goal title" /><select class="select" bind:value={form.goal_type}><option value="raise_funding">Raise funding</option><option value="find_advisors">Find advisors</option><option value="validate_market">Validate market</option><option value="build_operator_network">Build operator network</option></select></div><textarea class="textarea" bind:value={form.summary} rows="3" placeholder="Goal summary" style="margin-top:0.75rem;"></textarea><div style="display:flex; justify-content:space-between; margin-top:0.75rem;"><div class="muted">{error}</div><button class="button" on:click={submit} disabled={!form.title}>Add venture goal</button></div></SectionCard><SectionCard title="Goal List" subtitle="Goals drive relationship mapping and workflow actions.">{#if projects.length}<table class="table"><thead><tr><th>Title</th><th>Goal</th><th>Status</th><th>Summary</th></tr></thead><tbody>{#each projects as project}<tr><td>{project.title}</td><td>{project.goal_type}</td><td>{project.status}</td><td>{project.summary || 'No summary'}</td></tr>{/each}</tbody></table>{:else}<EmptyState title="No venture goals yet" body="Define the venture goal first, then map relationships around it." />{/if}</SectionCard></div>
